...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    if (ESTIMATE_EXTRINSIC)
    {
        ...
        {
            //ROS_DEBUG("calibration result for camera %d", k);
            ROS_DEBUG_STREAM("extirnsic tic: " << estimator.tic[k].transpose());
            ROS_DEBUG_STREAM("extrinsic ric: " << Utility::R2ypr(estimator.ric[k]).transpose());

            Eigen::Matrix4d eigen_T = Eigen::Matrix4d::Identity();
            eigen_T.block<3, 3>(0, 0) = estimator.ric[k];
            eigen_T.block<3, 1>(0, 3) = estimator.tic[k];
            cv::Mat cv_T;
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        odometry.header.frame_id = world_frame_id;
        odometry.child_frame_id = world_frame_id;
        Quaterniond tmp_Q;
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    br.sendTransform(tf::StampedTransform(transform, header.stamp, world_frame_id, "body"));
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {
                ...
                p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].point.x()); //都用的是左图的3d点和uv
