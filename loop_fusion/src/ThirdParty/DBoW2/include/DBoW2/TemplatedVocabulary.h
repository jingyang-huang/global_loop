...
namespace DBoW2
{
  ...
  typedef std::map<DBoW2::WordId, std::vector<int>> WordIdToFeatures;
  ...
  public:
    ...
    /**
     * Transform a set of descriptors into a bow vector and a map, the key
     * of the map is NodeId, its value are the indeces of features
     * @param features
     * @param v (out) bow vector
     * @param wf (out) map of NodeId and feature indexes
     */
    // virtual void transform(const std::vector<TDescriptor>& features,
    //   BowVector &v, WordIdToFeatures &wf) const;
  ...
  //   const std::vector<TDescriptor>& features, BowVector &v, WordIdToFeatures& wf) const
  // {
  //   v.clear();
  //   wf.clear();
