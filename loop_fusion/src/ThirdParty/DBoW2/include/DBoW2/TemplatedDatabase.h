...
namespace DBoW2
{
  ...
  public:
    ...
    // nearby search
    void setQueryPosGuess(Eigen::Vector3d pos)
    {
      entry_pos = pos;
      if_pos_set = true;
    }

    void addRefPos(Eigen::Vector3d pos)
    {
      EntryId entry_id = m_nentries;
      pos_map[entry_id] = pos;
      pos_list.push_back(pos);
    }

    void calcNeighborScore(const std::vector<TDescriptor> &kf_features, const std::vector<TDescriptor> &neighborkf_features, double &neighbor_score)
    {
      BowVector kf_vec, neighbor_vec;
      m_voc->transform(kf_features, kf_vec);
      m_voc->transform(neighborkf_features, neighbor_vec);
      neighbor_score = m_voc->score(kf_vec, neighbor_vec);
    }
  ...
  protected:
    Eigen::Vector3d entry_pos;
    std::vector<Eigen::Vector3d> pos_list;
    std::map<EntryId, Eigen::Vector3d> pos_map;
    bool if_pos_set = false;
  ...
  template <class TDescriptor, class F>
  TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const TemplatedDatabase<TDescriptor, F> &db)
      : m_voc(NULL)
  {
    *this = db;
  }
  ...
  template <class TDescriptor, class F>
  TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const std::string &filename)
      : m_voc(NULL)
  {
    load(filename);
  }
  ...
  template <class TDescriptor, class F>
  TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const char *filename)
      : m_voc(NULL)
  {
    load(filename);
  }
  ...
  template <class TDescriptor, class F>
  TemplatedDatabase<TDescriptor, F> &TemplatedDatabase<TDescriptor, F>::operator=(const TemplatedDatabase<TDescriptor, F> &db)
  {
    if (this != &db)
    {
      m_dfile = db.m_dfile;
                                                  ...
                                                  QueryResults &ret, int max_results, int max_id) const
  {
    ...
    for (vit = vec.begin(); vit != vec.end(); ++vit)
    {
      ...
      for (rit = row.begin(); rit != row.end(); ++rit)
      {
        ...
        // if dis( pos_list(entry_id) - pos) > 20, continue
        assert(m_nentries == pos_list.size());
        if (if_pos_set && entry_id < pos_list.size())
        {
          Eigen::Vector3d pos = pos_list[entry_id];
          if ((pos - entry_pos).norm() > 20)
            continue;
        }
                                               ...
                                               const std::string &name)
  {
    ...
    if (!m_voc)
      m_voc = new TemplatedVocabulary<TDescriptor, F>;
