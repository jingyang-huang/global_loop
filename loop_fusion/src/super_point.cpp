...
bool SuperPoint::construct_network(SampleUniquePtr<nvinfer1::IBuilder> &builder,
...
bool SuperPoint::infer_withTrack(const cv::Mat &image_, FeatureData &track_featuredata, Eigen::Matrix<float, 259, Eigen::Dynamic> &extract_features, Eigen::Matrix<float, 259, Eigen::Dynamic> &track_features)
{
    if (!context_)
    {
        context_ = SampleUniquePtr<nvinfer1::IExecutionContext>(engine_->createExecutionContext());
    ...
    if (!process_output_withTrack(buffers, track_featuredata, extract_features, track_features))
    {
        return false;
    }
                              ...
                              int h, int w, float threshold, int border, int top_k)
{
    // heat_map -> heat_img
    // cv::Mat heat_img;
    // // 将 heat_map 数据转换为 cv::Mat
    // cv::Mat heat_map_mat(h, w, CV_32F, const_cast<float *>(heat_map));
    // // 将 heat_map 归一化到 0-255 范围
    // cv::normalize(heat_map_mat, heat_map_mat, 0, 255, cv::NORM_MINMAX);
    // // 将 heat_map 转换为 8 位单通道图像
    // heat_map_mat.convertTo(heat_img, CV_8U);
    // // 应用颜色映射，将单通道图像转换为彩色图像
    // // cv::applyColorMap(heat_img, heat_img, cv::COLORMAP_JET);
    // cv::imshow("Heat Image", heat_img);
    // cv::waitKey(0);
...
void SuperPoint::track_point(const float *heat_map, FeatureData &track_featuredata, Eigen::Matrix<float, 259, Eigen::Dynamic> &track_features,
                             int h, int w, int border)
{
    std::vector<float> scores_v;
    std::vector<float> kpt_xs, kpt_ys;
    auto track_size = track_featuredata.size();
    scores_v.reserve(track_size);
    kpt_xs.reserve(track_size);
    kpt_ys.reserve(track_size);
    track_features.resize(259, track_size);
    ...
    {
        int x = round(track_featuredata[i].keypoint_2d_uv.pt.x / w_scale); //track pts to spp image pts
        int y = round(track_featuredata[i].keypoint_2d_uv.pt.y / h_scale);
        int idx = y * w + x;
        float score = *(heat_map + idx);
        track_features(0, i) = score;
        track_features(1, i) = x;
        track_features(2, i) = y;
    }

    // for (int i = 0; i < heat_map_size; ++i)
    // {
    //     // if (*(heat_map + i) < threshold)
    //     //     continue; // modify
...
bool SuperPoint::keypoints_decoder(const float *scores, const float *descriptors, Eigen::Matrix<float, 259, Eigen::Dynamic> &features)
{
    ...
    // scale back to original size(uv, first and second row)
...
bool SuperPoint::process_output_withTrack(const BufferManager &buffers, FeatureData &track_featuredata, Eigen::Matrix<float, 259, Eigen::Dynamic> &extract_features, Eigen::Matrix<float, 259, Eigen::Dynamic> &track_features)
{
    keypoints_.clear();
    descriptors_.clear();
    auto *output_score = static_cast<float *>(buffers.getHostBuffer(super_point_config_.output_tensor_names[0]));
    ...
    detect_point(output_score, extract_features, resized_height, resized_width, super_point_config_.keypoint_threshold,
                 super_point_config_.remove_borders, super_point_config_.max_keypoints);
    extract_descriptors(output_desc, extract_features, resized_height / 8, resized_width / 8, 8);
    // scale back to original size(uv, first and second row)
    extract_features.block(1, 0, 1, extract_features.cols()) = extract_features.block(1, 0, 1, extract_features.cols()) * w_scale;
    extract_features.block(2, 0, 1, extract_features.cols()) = extract_features.block(2, 0, 1, extract_features.cols()) * h_scale;

    // add track point
    if (track_featuredata.size())
    {
        track_point(output_score, track_featuredata, track_features, resized_height, resized_width, super_point_config_.remove_borders);
        extract_descriptors(output_desc, track_features, resized_height / 8, resized_width / 8, 8);
        track_features.block(1, 0, 1, track_features.cols()) = track_features.block(1, 0, 1, track_features.cols()) * w_scale;
        track_features.block(2, 0, 1, track_features.cols()) = track_features.block(2, 0, 1, track_features.cols()) * h_scale;
    }
...
void SuperPoint::save_engine()
{
    ...
    if (engine_ != nullptr)
    {
        ...
        std::cout << "super_point save engine success!" << std::endl;
    }
}
